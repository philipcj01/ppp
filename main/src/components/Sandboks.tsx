...
const Sandboks: React.FC = () => {
  const [selectedSolution, setSelectedSolution] = useState<SolutionType>('dokumentfortolker');
      ...
      default:
        return (
          <div className="no-solution-selected">
            ...
            <p>Brug dropdown menuen til at vælge hvilken AI-løsning du vil teste.</p>
          </div>
        );
    }
  };
      ...
      <section className="sandbox-controls">
        <div className="container">
          <div className="solution-selector">
            <label htmlFor="solution-dropdown">Vælg AI-løsning:</label>
              ...
              {isDropdownOpen && (
                <div className="dropdown-menu">
                  {solutions.map((solution) => (
                    <button
                      key={solution.id}
                      className={`dropdown-item ${selectedSolution === solution.id ? 'selected' : ''}`}
                      onClick={() => handleSolutionChange(solution.id)}
                    >
                      <div className="solution-info">
                        <span className="solution-name">{solution.name}</span>
                        <span className="solution-desc">{solution.description}</span>
                      </div>
                    </button>
                  ))}
                </div>
